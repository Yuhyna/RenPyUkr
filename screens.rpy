...
## два параметри, хто і що, ім'я персонажа, що говорить, і текст, який буде
...
## ідентифікаторами "window" для застосування властивостей стилю.
##
...
screen say(who, what):
    ...
    ## Якщо є зображення збоку, відобразіть його над текстом. Не відображати на
    ## телефоні варіант - немає місця.
    if not renpy.variant("small"):
        add SideImage() xalign 0.0 yalign 1.0


## Робить поле імен доступним для стилізації через об'єкт Character.
init python:
...
## використовується для передачі текстового підказки.
##
...
## командою меню. Один параметр, items, — це список об’єктів, кожен із полями
## заголовка та дії.
##
...
screen quick_menu():
    ...
    if quick_menu:
        ...
        hbox:
            ...
            textbutton _("Шв. збереження") action QuickSave()
            textbutton _("Шв. завантаження") action QuickLoad()
...
screen navigation():
    ...
    vbox:
        ...
        if main_menu:
            ...
            textbutton _("Почати гру") action Start()
        ...
        if renpy.variant("pc"):
            ...
            ## Кнопка виходу заборонена на iOS і непотрібна на Android і в
            ## Інтернеті.
...
## заголовка екрана та відображає фон, заголовок і навігацію.
##
## Параметр прокручування може бути None або одним із "viewport" чи "vpgrid".
## Коли цей екран призначений для використання з одним або декількома дітьми,
## які включені (розміщені) всередині нього.
...
screen about():
    ...
    ## game_menu.
    use game_menu(_("Про"), scroll="viewport"):
...
screen file_slots(title):
    ...
    default page_name_value = FilePageNameInputValue(pattern=_("Сторінка {}"), auto=_("Автозбереження"), quick=_("Швидкі збереження"))

    use game_menu(title):

        fixed:

            ## Це гарантує, що введення отримає подію enter раніше, ніж будь-яка
            ## кнопка.
            order_reverse True
            ...
            grid gui.file_slot_cols gui.file_slot_rows:
                ...
                for i in range(gui.file_slot_cols * gui.file_slot_rows):
                    ...
                    button:
                        ...
                        text FileTime(slot, format=_("{#file_time}%A, %d %B %Y, %H:%M"), empty=_("порожній слот")):
            ...
            hbox:
                ...
                if config.has_quicksave:
                    textbutton _("{#quick_page}Шв") action FilePage("quick")

                ## діапазон(1, 10) дає числа від 1 до 9.
...
## Екран налаштувань ###########################################################
##
## Екран налаштувань дозволяє гравцеві налагодити гру на свій смак.
##
        ...
        vbox:
            ...
            hbox:
                ...
                if renpy.variant("pc") or renpy.variant("web"):
                    ...
                    vbox:
                        style_prefix "radio"
                        label _("Режим гри")
                        textbutton _("Віконний") action Preference("display", "window")
                        textbutton _("Повноекранний") action Preference("display", "fullscreen")

                vbox:
                    style_prefix "check"
                    label _("Пропускати")
                    textbutton _("Новий текст") action Preference("skip", "toggle")
            ...
            hbox:
                ...
                vbox:
                    ...
                    label _("Затримка під час авточитання")
...
screen history():
    ...
    ## Уникайте прогнозування цього екрана, оскільки він може бути дуже великим.
    predict False

    use game_menu(_("Історія"), scroll=("vpgrid" if gui.history_height else "viewport"), yinitial=1.0):
        ...
        for h in _history_list:
            ...
            window:
                ...
                ## Це правильно розкладає речі, якщо history_height дорівнює
                ## None.
                has fixed:
                    yfit True
                ...
                if h.who:
                    ...
                    label h.who:
                        ...
                        ## Взяти колір тексту що від персонажа, якщо
                        ## встановлено.
...
screen help():
    ...
    use game_menu(_("Допомога"), scroll="viewport"):
        ...
        vbox:
            ...
            hbox:
                ...
                textbutton _("Мишка") action SetScreenVariable("device", "mouse")
...
screen keyboard_help():
    ...
    hbox:
        label _("Enter")
        text _("Розвиває діалог та активує виділені кнопки.")

    hbox:
        label _("Пробіл")
        text _("Розвиває діалог без вибору варіантів.")
    ...
    hbox:
        label _("Page Up")
        text _("Відкат до попередньої репліки..")

    hbox:
        label _("Page Down")
        text _("Перехід до наступної репліки.")
    ...
    hbox:
        label "S"
        text _("Робить скриншот.")

    hbox:
        label "V"
        text _("Вмикає допоміжне {a=https://www.renpy.org/l/voicing}озвучення{/a}.")

    hbox:
        label "Shift+A"
        text _("Відкриває меню спеціальних можливостей.")
...
screen mouse_help():
    ...
    hbox:
        label _("Середня кнопка")
        text _("Приховує інтерфейс.")

    hbox:
        label _("Права кнопка")
        text _("Відкриває меню гри.")

    hbox:
        label _("Колесико вгору")
        text _("Відкат до попередньої репліки.")
...
screen nvl(dialogue, items=None):
    ...
    window:
        ...
        ## для config.narrator_menu встановлено значення True.
        for i in items:
...
screen quick_menu():
    ...
    if quick_menu:
        ...
        hbox:
            ...
            textbutton _("Назад") action Rollback()
            textbutton _("Приховати") action Skip() alternate Skip(fast=True, confirm=True)
            textbutton _("Авто") action Preference("auto-forward", "toggle")
